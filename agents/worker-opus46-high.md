---
name: worker-opus46-high
description: High-effort routed worker pinned to Opus 4.6 for fable-router. Used when the Opus version pin is set to 4.6 — tricky debugging, cross-file refactors, retry after failed validation, and consequential independent second-opinion review, on claude-opus-4-6 regardless of what the opus alias resolves to. Spawn without the Agent tool's model parameter so the pin holds.
